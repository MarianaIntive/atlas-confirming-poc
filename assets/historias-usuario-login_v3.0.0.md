...
> **Versión:** v3.0.0 · **Fecha:** 2026-08-03
> **Fuente única de requerimientos:** `login (3) (1).xlsx` — hoja **LOGIN** (filas 3 a 42), complementada con las hojas *Matriz de trazabilidad*, *API REST — Backend dominio (Identity)* y *API REST — BFF (orientada a UI)*.
...
> **Nota de regeneración:** Documento regenerado con la skill `po-expert-user-stories` a partir del Excel adjunto `login (3) (1).xlsx`. Se validó fila a fila (incl. *strikethrough*) la matriz de inclusión/desestimación; el **alcance comprometido coincide** con `historias-usuario-login_v2.0.0.md` (mismo set de HU/HT/TAREA, RN, MSG, spikes y recomendaciones).

---

## Tabla de contenidos

0. [Qué cambia respecto de v2.0.0](#0-qué-cambia-respecto-de-v200)
...
## 0. Qué cambia respecto de v2.0.0

Cambios de **fuente y trazabilidad** (el alcance, filas incluidas/desestimadas, RN, MSG, spikes, R-01..R-15, endpoints, AC y Gherkin son los mismos que v2.0.0):

1. **Fuente de requerimientos** actualizada a `login (3) (1).xlsx` (antes `login (2).xlsx`).
2. **Validación mecánica de tachados**: se confirmó el *strikethrough* celda a celda en la hoja LOGIN (filas 3–42); la decisión de inclusión/desestimación de §2 se mantiene.
3. Formato de escritura **sin cambios** respecto de v2.0.0 (tarjeta de backlog, AC numerados, Gherkin en español, INVEST), según skill `po-expert-user-stories`.
4. Resumen del backlog producido: **11 HU** · **10 HT** · **4 TAREA** · **6 spikes** · **15 recomendaciones** (fuera del Excel).
...
Hoja `LOGIN` de `login (3) (1).xlsx`. Se listan las 40 filas con contenido (filas 3 a 42). La columna **Estado** refleja el *strikethrough* del Excel: si el Summary (o el Issue Key cuando aplica) está tachado ⇒ ❌ Desestimada.
